...
export function CoupleChat({ linkId, myNumeroH, partnerLabel }: Props) {
  const [messages, setMessages] = useState<CoupleMessageItem[]>([])
  ...
  const loadMessages = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const res = await fetch(`${API_BASE}/api/couple/messages?linkId=${encodeURIComponent(linkId)}`, {
  ...
  // Temps réel : rejoint la room privée de ce couple
  useEffect(() => {
    if (!linkId) return
    const socket = getSocket()
    socket.emit('join-room', `couple-${linkId}`)
    const onMsg = (msg: CoupleMessageItem) => {
      setMessages(prev => {
    ...
    socket.on('couple-message', onMsg)
    return () => { socket.off('couple-message', onMsg) }
  }, [linkId])
  ...
  const sendMessage = async () => {
    ...
    setIsSending(true)
    try {
      const res = await fetch(`${API_BASE}/api/couple/messages`, {
        method: 'POST',
  ...
  const sendMediaMessage = useCallback(async (file: File) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/couple/messages/upload`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token()}` },
        body: formData
      })
      ...
      {messages.length > 0 && (
        <div className="flex gap-1.5 overflow-x-auto px-3 py-2 bg-white border-b border-gray-100 flex-shrink-0">
          <button
            ...
            className={`flex-shrink-0 px-3 py-1 rounded-full text-xs font-medium transition-colors ${feedFilter === 'all' ? 'bg-rose-600 text-white' : 'bg-gray-100 text-gray-600'}`}
          >
            💬 Tout
          </button>
          {COUPLE_CATEGORIES.map(cat => (
            <button
              ...
              className={`flex-shrink-0 px-3 py-1 rounded-full text-xs font-medium transition-colors ${feedFilter === cat.id ? 'bg-rose-600 text-white' : 'bg-gray-100 text-gray-600'}`}
            >
              {cat.icon} {cat.label}
            </button>
          ))}
                ...
                return (
                  <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-xs sm:max-w-md px-3 py-2 rounded-2xl shadow-sm ${isMe ? 'bg-rose-500 text-white rounded-br-sm' : 'bg-white text-gray-900 rounded-bl-sm'}`}>
                      {(msg.messageType === 'text' || !msg.messageType) && (
                        <p className="text-sm whitespace-pre-line">
                          {msg.category && msg.category !== 'information' && (
                            <span className="mr-1">{COUPLE_CATEGORIES.find(c => c.id === msg.category)?.icon}</span>
                          )}
                          {msg.content}
                        </p>
                      )}
                      ...
                      <p className={`text-[10px] mt-1 ${isMe ? 'text-rose-100' : 'text-gray-500'}`}>
      ...
      <div className="border-t border-gray-200 bg-gray-50 px-3 py-2 flex-shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 min-w-0 relative">
            ...
            >
              <span className="text-base leading-none">{COUPLE_CATEGORIES.find(c => c.id === category)?.icon}</span>
            </button>
            <input
              type="text"
              value={newMessage}
          ...
          {newMessage.trim() && (
            <button
              ...
              onClick={sendMessage}
              disabled={isSending}
              className="w-10 h-10 rounded-full flex items-center justify-center text-white text-lg bg-rose-600 hover:bg-rose-700 transition-colors"
            >
              ➤
            </button>
          )}
