...
export function FriendChat({ linkId, myNumeroH, partnerLabel }: Props) {
  ...
  const loadMessages = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const res = await fetch(`${API_BASE}/api/friends/messages?linkId=${encodeURIComponent(linkId)}`, {
  ...
  useEffect(() => {
    ...
    socket.emit('join-room', `friend-${linkId}`)
    const onMsg = (msg: FriendMessageItem) => {
      setMessages(prev => {
    ...
    socket.on('friend-message', onMsg)
    return () => { socket.off('friend-message', onMsg) }
  }, [linkId])
  ...
  const sendMessage = async () => {
    ...
    setIsSending(true)
    try {
      const res = await fetch(`${API_BASE}/api/friends/messages`, {
        method: 'POST',
  ...
  const sendMediaMessage = useCallback(async (file: File) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/friends/messages/upload`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token()}` },
        body: formData
      })
  ...
  return (
    <div className="flex flex-col border border-slate-200 rounded-xl overflow-hidden" style={{ minHeight: 420, maxHeight: '75vh' }}>
      {messages.length > 0 && (
        <div className="flex gap-1.5 overflow-x-auto px-3 py-2 bg-white border-b border-gray-100 flex-shrink-0">
          <button
            onClick={() => setFeedFilter('all')}
            className={`flex-shrink-0 px-3 py-1 rounded-full text-xs font-medium transition-colors ${feedFilter === 'all' ? 'bg-violet-600 text-white' : 'bg-gray-100 text-gray-600'}`}
          >
            💬 Tout
          </button>
          {FRIEND_CATEGORIES.map(cat => (
            <button
              ...
              className={`flex-shrink-0 px-3 py-1 rounded-full text-xs font-medium transition-colors ${feedFilter === cat.id ? 'bg-violet-600 text-white' : 'bg-gray-100 text-gray-600'}`}
            >
              {cat.icon} {cat.label}
            </button>
          ))}
              ...
              .map((msg) => {
                ...
                return (
                  <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-xs sm:max-w-md px-3 py-2 rounded-2xl shadow-sm ${isMe ? 'bg-violet-500 text-white rounded-br-sm' : 'bg-white text-gray-900 rounded-bl-sm'}`}>
                      {(msg.messageType === 'text' || !msg.messageType) && (
                        <p className="text-sm whitespace-pre-line">
                          {msg.category && msg.category !== 'information' && (
                            <span className="mr-1">{FRIEND_CATEGORIES.find(c => c.id === msg.category)?.icon}</span>
                          )}
                          {msg.content}
                        </p>
                      )}
                      ...
                      <p className={`text-[10px] mt-1 ${isMe ? 'text-violet-100' : 'text-gray-500'}`}>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 min-w-0 relative">
            <button
              ...
              className={`absolute left-1 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full overflow-hidden flex items-center justify-center transition-colors ${showCategoryGrid ? 'bg-violet-100' : 'hover:bg-gray-100'}`}
            >
              <span className="text-base leading-none">{FRIEND_CATEGORIES.find(c => c.id === category)?.icon}</span>
            </button>
            <input
              type="text"
              value={newMessage}
              ...
              className="w-full min-w-0 pl-10 pr-4 py-2 rounded-full border border-gray-300 focus:outline-none focus:ring-2 focus:ring-violet-500 bg-white text-sm"
            />
            {showCategoryGrid && (
              <div className="absolute bottom-11 left-0 z-20 bg-white rounded-xl shadow-lg border border-gray-200 p-2 grid grid-cols-4 gap-1 w-56">
                {FRIEND_CATEGORIES.map(cat => (
                  <button
                    ...
                    className={`flex flex-col items-center gap-0.5 py-2 rounded-lg ${category === cat.id ? 'bg-violet-100' : 'hover:bg-gray-100'}`}
                    title={cat.label}
                  >
          ...
          {newMessage.trim() && (
            <button
              ...
              onClick={sendMessage}
              disabled={isSending}
              className="w-10 h-10 rounded-full flex items-center justify-center text-white text-lg bg-violet-600 hover:bg-violet-700 transition-colors"
            >
              ➤
            </button>
          )}
