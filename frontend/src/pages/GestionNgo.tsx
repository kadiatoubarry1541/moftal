  ...
  return (
    <div className="gestion-page" style={{ maxWidth: 960, margin: "0 auto", padding: "0 0 60px" }}>
      <DynamicAppManifest
        ...
        description={`Gestion ONG — ${tenant?.name || ""}`}
        startUrl={`/gestion-ngo/${tenantCode}`}
        themeColor={ROSE}
      />
      <style>{`@keyframes spin{to{transform:rotate(360deg)}} @keyframes fadeIn{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:translateY(0)}}`}</style>

      {/* ── Header ── */}
      <div className="gestion-pad" style={{ background: "linear-gradient(135deg,#be123c,#e11d48)", padding: "28px 28px 0" }}>
