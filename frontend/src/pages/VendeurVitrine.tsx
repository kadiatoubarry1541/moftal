...
export default function VendeurVitrine() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/pro-public/vendor/${tenantCode}`).then(r => r.json()),
      fetch(`${API}/api/pro-public/vendor/${tenantCode}/data`).then(r => r.json()),
    ]).then(([t, d]) => {
      if (t.success) setVendor(t.tenant);
      else setError(t.message || "Vendeur introuvable");
      if (d.success) setData(d);
    }).catch(() => setError("Impossible de charger ce vendeur."))
  ...
  if (error || !vendor) return (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"center", height:"100vh" }}>
      <div style={{ textAlign:"center", maxWidth:400, padding:"0 24px" }}>
        ...
        <h2 style={{ fontSize:20, fontWeight:700, marginBottom:8 }}>Vendeur introuvable</h2>
  ...
  return (
    <div style={{ fontFamily:"system-ui,sans-serif", minHeight:"100vh", background:"#f8fafc" }}>
      ...
      <div style={{ background:`linear-gradient(135deg,#0369a1,${COLOR})`, padding:"32px 24px 28px", position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ maxWidth:900, margin:"0 auto" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:20 }}>
            {vendor.logo_url
              ? <img src={vendor.logo_url} alt="logo" style={{ width:72, height:72, borderRadius:14, objectFit:"cover", border:"3px solid rgba(255,255,255,0.3)" }} />
              : <div style={{ width:72, height:72, borderRadius:14, background:"rgba(255,255,255,0.2)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:36 }}>🛍️</div>}
            <div>
              <h1 style={{ margin:0, color:"white", fontSize:26, fontWeight:800 }}>{vendor.name}</h1>
              <div style={{ color:"rgba(255,255,255,0.8)", fontSize:14, marginTop:4 }}>Boutique / Détaillant</div>
              {vendor.city && <div style={{ color:"rgba(255,255,255,0.7)", fontSize:13, marginTop:2 }}>📍 {vendor.city}</div>}
            </div>
          </div>
          <div style={{ display:"flex", gap:16, marginTop:24, flexWrap:"wrap" }}>
            {[
              { label:"Articles", value: stats.products ?? products.length, icon:"🛍️" },
      ...
      <div style={{ maxWidth:900, margin:"0 auto", padding:"28px 20px" }}>
        {(vendor.description || vendor.phone || vendor.email || vendor.address) && (
          <div style={{ background:"white", borderRadius:14, border:"1px solid #e2e8f0", padding:"20px 24px", marginBottom:24, animation:"fadeIn 0.2s ease" }}>
            ...
            {vendor.description && <p style={{ margin:"0 0 12px", fontSize:14, color:"#475569", lineHeight:1.7 }}>{vendor.description}</p>}
        ...
        <div>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:14, flexWrap:"wrap", gap:10 }}>
            <h2 style={{ margin:0, fontSize:17, fontWeight:800, color:"#0f172a" }}>🛍️ Articles ({products.length})</h2>
          ...
          {filtered.length === 0
            ? <div style={{ textAlign:"center", padding:"48px 0", color:"#94a3b8" }}><div style={{ fontSize:40, marginBottom:12 }}>🛍️</div><div>Aucun article trouvé</div></div>
            : <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))", gap:14 }}>
                {filtered.map((p: any, i: number) => (
                  <div key={i} style={{ background:"white", border:"1px solid #e2e8f0", borderRadius:12, overflow:"hidden", animation:`fadeIn 0.2s ease ${i*0.04}s both` }}>
        ...
        {(vendor.phone || vendor.email) && (
          <div style={{ marginTop:32, background:`linear-gradient(135deg,#0369a1,${COLOR})`, borderRadius:16, padding:"24px 28px", color:"white" }}>
            <h3 style={{ margin:"0 0 8px", fontSize:17, fontWeight:800 }}>Contacter ce vendeur</h3>
            <div style={{ display:"flex", gap:12, flexWrap:"wrap", marginTop:16 }}>
              {vendor.phone && <a href={`tel:${vendor.phone}`} style={{ display:"flex", alignItems:"center", gap:8, padding:"10px 20px", background:"white", color: COLOR, borderRadius:10, textDecoration:"none", fontWeight:700, fontSize:14 }}>📞 {vendor.phone}</a>}
              {vendor.email && <a href={`mailto:${vendor.email}`} style={{ display:"flex", alignItems:"center", gap:8, padding:"10px 20px", background:"rgba(255,255,255,0.15)", color:"white", borderRadius:10, textDecoration:"none", fontWeight:700, fontSize:14, border:"1px solid rgba(255,255,255,0.3)" }}>✉️ {vendor.email}</a>}
            </div>
          </div>
        )}
      </div>
      <ProPublicationsWidget tenantCode={tenantCode!} accentColor="#0369a1" accentDark="#0c4a6e" />
    </div>
  );
}
