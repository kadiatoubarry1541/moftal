...
export default function GuineeForestiere() {
  ...
  const loadGroups = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE}/api/regions/groups?region=Guinée forestière`, {
        headers: {
  ...
  const createEvent = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${API_BASE}/api/regions/guinee-forestiere/events`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const createAnnouncement = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${API_BASE}/api/regions/guinee-forestiere/announcements`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
