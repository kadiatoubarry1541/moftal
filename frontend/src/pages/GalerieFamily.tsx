  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      ...
      if (file.type.startsWith('video/')) {
        ...
        if (videoDuration > 60) {
          setError('La vidéo est trop longue. Maximum 1 minute pour la galerie familiale.')
          setUploading(false)
          return
        }
      }
            ...
            {quota && (
              <div className="mb-4 p-3 bg-gray-50 border border-gray-200 rounded-xl text-xs">
                ...
                {quota.photosRestantes === 0 && points > 0 && (
                  <p className="mt-1.5 text-gray-500">Photos : <strong>1 pt</strong> · Vidéo ≤10s : <strong>2 pts</strong> · ≤20s : <strong>3 pts</strong> · au-delà (jusqu'à 1 min) : <strong>5 pts</strong></p>
                )}
              </div>
            )}
            <div className="mb-4">
